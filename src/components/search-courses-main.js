...
class SearchCourse extends Component {
    ...
    returnURL(data){
        ...
        if(name){
            return `http://localhost:8080/course/${name}`
        }else if(data.location.type === 'zip'){
            return `http://localhost:8080/course/zip/${data.location.value}/${data.location.radius}`;
        }else if(location){
            return `http://localhost:8080/course/${data.location.type}/${data.location.value}`;
        }else if(data.course_id){
            return `http://localhost:8080/course/${data.course_id}`;
        }else{
            location ? location = data.location.type : location = 'United States';
            return `http://localhost:8080/course/country/${location}`;
        }
    }
    render() {
        return (
