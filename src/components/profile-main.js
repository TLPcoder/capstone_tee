...
class ProfileMain extends Component {
    ...
    render() {
        ...
        var bidURL = `http://localhost:8080/user/bids/${userId}`;
        if (userId === null) {
            return (
        ...
        return (
            <div id="profile-body-container">
                ...
                <DisplayWindow profile={true} category ={'Favorites'} user ={this.getUserId} url={`http://localhost:8080/user/favorites/${userId}`}/>
            <DisplayWindow profile={true} category ={'Current'} user ={this.getUserId} url={`http://localhost:8080/auction`}/>
                </div>
            </div>
        )
    }
