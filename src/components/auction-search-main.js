...
class AuctionSearch extends Component {
    ...
    yourAuctions() {
        if (!this.state.yourAuctions) {
            ...
            this.setState({yourAuctions: true,
                url: `http://localhost:8080/user/auctions/${user}`,
                key: this.state.key + 1
            });
        }else{
            this.setState({
    ...
    returnURL(data) {
        ...
        if (name) {
            return `http://localhost:8080/course/name/id/${name}/asc`;
        } else if (data.location.type === 'zip') {
            console.log("i am here for zip")
            return `http://localhost:8080/tee_time/${data.location.value}/${data.location.radius}/asc/asc`;
        } else if (location) {
            return `http://localhost:8080/tee_time/${data.location.type}/${data.location.value}/asc/asc`;
        } else {
            return `http://localhost:8080/tee_time/country/United States/asc/asc`;
        }
    }
    render() {
