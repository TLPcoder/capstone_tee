...
class AutoSelect extends Component {
    ...
    getCourseNames() {
        fetch(`http://localhost:8080/auction/course/name/id`).then((promise) => {
            return promise.json();
        }).then((json) => {
            this.setState({
                courseNames: json
    ...
    createAuction() {
        fetch(`http://localhost:8080/auction/create`, {
            method: "POST",
            headers: {
    ...
    createBid(data){
        ...
        fetch(`http://localhost:8080/payment/newBid`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json'
